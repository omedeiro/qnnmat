classdef labjack < handle
    ...
    methods
        function obj = labjack()
            ...
            [obj.Error, obj.ljHandle] = obj.OpenLabJack(obj.LJ_dtU6,obj.LJ_ctUSB,'1',1); % Returns ljHandle for open LabJack

        end
        
        function [Error, ljHandle] = OpenLabJack(obj, DeviceType,ConnectionType,Address,FirstFound)
        ...
        function Error_Message(obj,Error)
            if(Error ~= 0)
                ErrorString = ErrorToString(Error)
            end
        end
        
        function [ljError] = AddRequest(obj,ljHandle, IOType, Channel, ljValue, x1, ljUserData)
            [ljError] = calllib('labjackud','AddRequest',ljHandle,IOType,Channel,ljValue,x1,ljUserData);
        end
        
        function [Error, ljString] = DoubleToStringAddress(obj, Number,HexDot)
            String = '';
            [Error, ljString] = calllib('labjackud','DoubleToStringAddress',Number,String,HexDot);
        end

        
        function [ljError ljValue] = GetResult(obj, ljHandle, IOType, Channel, ljValue)
            [ljError ljValue] = calllib('labjackud','GetResult',ljHandle,IOType,Channel,ljValue);
        end
        
        
        function [ljError, ljValue] = eGet(obj, ljHandle, IOType, Channel, ljValue, x1);
