...
[Error ljHandle] = ljud_OpenLabJack(LJ_dtU6,LJ_ctUSB,'1',1);
...
% Configure AIN0 with Bipolar ±10 volt range
Error = ljud_AddRequest(ljHandle,LJ_ioPUT_AIN_RANGE,0,LJ_rgBIP10V,0,0);
Error_Message(Error)

% Configure AIN1 with Bipolar ±10 volt range
Error = ljud_AddRequest(ljHandle,LJ_ioPUT_AIN_RANGE,1,LJ_rgBIP10V,0,0);
Error_Message(Error)

% Configure AIN2 with Bipolar ±10 volt range
Error = ljud_AddRequest(ljHandle,LJ_ioPUT_AIN_RANGE,2,LJ_rgBIP10V,0,0);
Error_Message(Error)

% Configure AIN3 with Bipolar ±10 volt range
Error = ljud_AddRequest(ljHandle,LJ_ioPUT_AIN_RANGE,3,LJ_rgBIP10V,0,0);
Error_Message(Error)
